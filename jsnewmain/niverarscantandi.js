...
function niverHoje(data) {
    ...
    data.map((pessoa) => {
        ...
        if (dia.includes(niverSplit[0]) && mes.includes(niverSplit[1]) && pessoa.coral.includes("Ars")) {
...
function niverMes(data) {
    ...
    data.map((pessoa) => {
        ...
        if (mes.includes(niverSplit[1]) && pessoa.coral.includes("Ars")) {
            aniversariantes.push(pessoa);
        }
    })
    if (aniversariantes.length>0) {
...
function compareFn(a, b) {
  ...
  function listaNiver(data) {
    ...
    data.map((pessoa) => {
        ...
        if (pessoa.coral.includes("Ars")) {
            aniversariantes2.push(pessoa);
        }
    })
    if (aniversariantes2.length>0) {
        textoH2Lista.innerText = `Lista de aniversários Ars Cantandi`;
    }
